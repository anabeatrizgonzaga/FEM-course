...
c ============================================================
c ALTERAÇÃO PARA SUPORTE 3D
c Agora o programa aceita problemas bidimensionais e tridimensionais.
c ============================================================
      if (ndm .ne. 2 .and. ndm .ne. 3) then
        print*, '*** ERRO: programa valido apenas para 2D (ndm=2) ou',
     .          ' 3D (ndm=3)'
        stop
      endif
c
...
c ======================================================================
      ...
      integer iaux(7)
      real*8 e(10,*),x(ndm,*),f(ndf,*),aux(7),dum(1)
c
...
c==============================================================
c ALTERAÇÃO PARA 3D
c Quando ndm=2 -> no x y
c Quando ndm=3 -> no x y z
c O laço abaixo utiliza ndm, portanto nenhuma alteração
c adicional foi necessária.
c==============================================================
      do 200 i = 1, nnode
...
c       iel = 7 => T4 Tetraedro linear 4 nós (Elasticidade 3D)
c ======================================================================
      integer nel,iel,ndm,nst,isw
      real*8 e(*),xl(*),ul(*),fl(*),sl(*)
      goto (100,200,300,400,500,600,700) iel
  ...
  600 call elmt01_axi(e,xl,ul,fl,sl,nel,ndm,nst,isw,nin)
      return
  700 call elmt05(e,xl,ul,fl,sl,nel,ndm,nst,isw,nin)
      end
...
c        nvec  => campo vetorial (para visualizacao 3D)
...
c
      ...
      do 100 i = 1, nnode
        if (ndm .eq. 2) then
          write(nout,'(i10,3e15.5)') i, x(1,i), x(2,i), 0.0d0
        else
          write(nout,'(i10,3e15.5)') i, x(1,i), x(2,i), x(3,i)
        endif
  100 continue
c
...
c
      ...
      do 300 i = 1, nnode
        do 310 j = 1, ndf
          ...
          if(k .gt. 0) aux(j) = u(k)
  310   continue
        write(nout,'(i10,6e15.5)') i,(aux(j),j=1,ndf)
  300 continue
c
c     Campo vetorial (garante 3 componentes para pos-processamento 3D):
c
      write(nout,'(a)') 'nvec '
      do 400 i = 1, nnode
        do 410 j = 1, ndf
  ...
  410   continue
        if (ndf .eq. 1) then
            write(nout,'(i10,3e15.5)') i, aux(1), 0.0d0, 0.0d0
        else if (ndf .eq. 2) then
            write(nout,'(i10,3e15.5)') i, aux(1), aux(2), 0.0d0
        else
            write(nout,'(i10,3e15.5)') i, aux(1), aux(2), aux(3)
        endif
  400 continue
c
      write(nout,'(a)') 'end '
...
c
      ...
      do 100 i = 1, nnode
        if(ndm.eq.2) then
            ...
            write(nvtk,'(3e16.8)') x(1,i), x(2,i), x(3,i)
        endif
  100 continue
c
...
c     5  = VTK_TRIANGLE (T3 - 2D)
c     9  = VTK_QUAD (Q4 - 2D)
c     10 = VTK_TETRA (T4 - 3D)
c
...
c        Se tem 4 nos e estamos em 3D, e um Tetraedro (10). Se for 2D, e Quadrilatero (9)
         do 350 i = 1, numel
           if (ndm .eq. 3) then
              write(nvtk,'(i2)') 10
           else
...
c
c     Escrita dos resultados baseada nos graus de liberdade (ndf)
c
      if (ndf .eq. 2) then
c
...
c
c     Dados nodais: 3D (ux, uy, uz)
c
         write(nvtk,'(a,i8)') 'POINT_DATA ', nnode
         write(nvtk,'(a)') 'VECTORS Deslocamentos3D float'
         do 600 i = 1, nnode
           aux(1) = f(1,i)
           aux(2) = f(2,i)
           aux(3) = f(3,i)
  ...
  600    continue
      ...
      subroutine elmt05(e,xl,ul,fl,sl,nel,ndm,nst,isw,nin)
      implicit none
c     
      integer ndm, nst, nel, isw, nin
      real*8  e(*), xl(ndm,*), ul(ndm,*), fl(*), sl(nst,nst)
c
c     Variaveis locais da rotina:
      real*8  x1, y1, z1, x2, y2, z2, x3, y3, z3, x4, y4, z4
      real*8  vol6, volume, e_mod, v_poi, factor, sum_val
      real*8  b1, c1, d1, b2, c2, d2, b3, c3, d3, b4, c4, d4
      real*8  bmat(6,12), dmat(6,6), btd(12,6)
      integer i, j, k

c     SUBROTINA PARA O ELEMENTO TETRAEDRO LINEAR DE 4 NOS - 12 GRAUS DE LIBERDADE
c     isw=1: leitura das constantes fisicas do material (E, nu)
c     Segue o mesmo padrao dos demais elementos (elmt01..elmt04):
c     isw = 1 (leitura) ou 2 (rigidez); 
      if (isw.eq.1) then
         read(nin,*) e(1), e(2)
         return
      endif
c
c     Qualquer isw diferente de 2 (montagem da rigidez) é ignorado
      if (isw.ne.2) return
c
c     Zerar a matriz de rigidez local para evitar lixo de memoria
      do i = 1, nst
         do j = 1, nst
            sl(i,j) = 0.0d0
         enddo
      enddo
c
c     Propriedades do material - lidas do arquivo .dat via isw=1
      e_mod = e(1) ! Modulo de Elasticidade
      v_poi = e(2) ! Coeficiente de Poisson
c
c     1. EXTRAIR COORDENADAS DOS NOS (1 a 4)
      x1 = xl(1,1); y1 = xl(2,1); z1 = xl(3,1)
      x2 = xl(1,2); y2 = xl(2,2); z2 = xl(3,2)
      x3 = xl(1,3); y3 = xl(2,3); z3 = xl(3,3)
      x4 = xl(1,4); y4 = xl(2,4); z4 = xl(3,4)
c
c     2. CALCULO DO DETERMINANTE DO VOLUME (6 * Volume)
      vol6 = x2*(y3*z4 - y4*z3) - x3*(y2*z4 - y4*z2) + 
     &       x4*(y2*z3 - y3*z2) - x1*(y3*z4 - y4*z3 - 
     &       y2*z4 + y4*z2 + y2*z3 - y3*z2) - 
     &       y1*(x2*z4 - x4*z2 - x3*z4 + x4*z3 + x3*z2 - x2*z3) +
     &       z1*(x2*y4 - x4*y2 - x3*y4 + x4*y3 + x3*y2 - x2*y3)
c
      volume = abs(vol6) / 6.0d0
      if (volume.le.1.0d-12) then
         print*, 'Erro: Elemento 3D com volume nulo ou degenerado!'
         stop
      endif
c
c     3. MATRIZ CONSTITUTIVA D
      factor = e_mod / ((1.0d0 + v_poi) * (1.0d0 - 2.0d0 * v_poi))
c
      do i = 1, 6
         do j = 1, 6
            dmat(i,j) = 0.0d0
      ...
      enddo
c
      dmat(1,1) = factor * (1.0d0 - v_poi)
      dmat(1,2) = factor * v_poi
      dmat(1,3) = factor * v_poi
      dmat(2,1) = factor * v_poi
      dmat(2,2) = factor * (1.0d0 - v_poi)
      dmat(2,3) = factor * v_poi
      dmat(3,1) = factor * v_poi
      dmat(3,2) = factor * v_poi
      dmat(3,3) = factor * (1.0d0 - v_poi)
c
      dmat(4,4) = factor * (0.5d0 - v_poi)
      dmat(5,5) = factor * (0.5d0 - v_poi)
      dmat(6,6) = factor * (0.5d0 - v_poi)
c
c     4. CALCULO DOS COFATORES (bi, ci, di) PARA AS DERIVADAS
...
c
c     5. MONTAGEM DA MATRIZ OPERADOR DE DEFORMACAO B (6 linhas x 12 colunas)
      do i = 1, 6
         do j = 1, 12
            bmat(i,j) = 0.0d0
         enddo
      enddo
c
c     No 1 (Colunas 1, 2, 3)
      bmat(1,1) = b1; bmat(2,2) = c1; bmat(3,3) = d1
      bmat(4,1) = c1; bmat(4,2) = b1
      bmat(5,2) = d1; bmat(5,3) = c1
      bmat(6,1) = d1; bmat(6,3) = b1
c
c     No 2 (Colunas 4, 5, 6)
      bmat(1,4) = b2; bmat(2,5) = c2; bmat(3,6) = d2
      bmat(4,4) = c2; bmat(4,5) = b2
      bmat(5,5) = d2; bmat(5,6) = c2
      bmat(6,4) = d2; bmat(6,6) = b2
c
c     No 3 (Colunas 7, 8, 9)
      bmat(1,7) = b3; bmat(2,8) = c3; bmat(3,9) = d3
      bmat(4,7) = c3; bmat(4,8) = b3
      bmat(5,8) = d3; bmat(5,9) = c3
      bmat(6,7) = d3; bmat(6,9) = b3
c
c     No 4 (Colunas 10, 11, 12)
      bmat(1,10) = b4; bmat(2,11) = c4; bmat(3,12) = d4
      bmat(4,10) = c4; bmat(4,11) = b4
      bmat(5,11) = d4; bmat(5,12) = c4
      bmat(6,10) = d4; bmat(6,12) = b4
c
c     Dividir bmat por (6 * Volume) para normalizar os gradientes reais
      do i = 1, 6
         do j = 1, 12
            bmat(i,j) = bmat(i,j) / vol6
         enddo
      enddo
c
c     6. OPERACOES MATRICIAIS: sl = (B^T * D * B) * Volume
c     Passo 6.1: btd = B^T * D  (Matriz 12x6)
      do i = 1, 12
         do j = 1, 6
            btd(i,j) = 0.0d0
            do k = 1, 6
               btd(i,j) = btd(i,j) + bmat(k,i) * dmat(k,j)
            enddo
         enddo
      enddo
c
c     Passo 6.2: sl = btd * B * Volume  (Matriz final 12x12)
      do i = 1, 12
         do j = 1, 12
            sum_val = 0.0d0
            do k = 1, 6
               sum_val = sum_val + btd(i,k) * bmat(k,j)
            enddo
            sl(i,j) = sum_val * volume
         enddo
      enddo
c
c     7. VETOR DE FORCAS INTERNAS DO ELEMENTO: fl = sl * ul
c     Necessario para addstf corrigir o vetor de forcas globais
c     quando ha graus de liberdade restritos com deslocamento
c     prescrito nao-nulo (ul). Sem isso, fl fica com lixo de
c     memoria (valor do elemento anterior), corrompendo o vetor
c     de forcas mesmo em problemas com apoios simples (u=0).
      call lku(sl,ul,fl,nst)
c
      return
      end