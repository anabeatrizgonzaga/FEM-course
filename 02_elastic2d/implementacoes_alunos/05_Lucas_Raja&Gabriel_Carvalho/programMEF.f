...
c     Adicionar leitura de aceleração de gravidade (gx, gy) por material 
c     Criar subrotina que calcula vetor de cargas de corpo para T3 e Q4, 
c     acumulando no vetor global de forças F.
...
c
     ...
     .          ndf,nvtk)
c
c     FECHAMENTO SEGURO DOS ARQUIVOS (No final da rotina):
c     Garante que o Windows descarregue o buffer e salve os dados reais.
c
      close(nout)
      close(nvtk)
c
...
c
c       Se o programa estiver na etapa de montagem (isw=2), calcula 
c       e acumula as forcas de corpo (gravidade) no vetor local fl
c
        if (isw .eq. 2) then
          call bodyforce(e(1,ma),xl,fl,nel,iel,ndm,nen,nst)
        endif
...
c ======================================================================
      subroutine bodyforce(e,xl,fl,nel,iel,ndm,nen,nst)
c     Calcula as forcas nodais equivalentes devidas a carga de corpo
c     (peso proprio via gravidade) para o elemento T3 e acumula em fl.
c ======================================================================
      integer nel,iel,ndm,nen,nst,i,k
      real*8  e(*),xl(ndm,*),fl(*)
      real*8  rho,gx,gy,thic,area,det,wt,rr,ss
      real*8  hr(4),hs(4),xj(2,2),g_shape(4)
c
c     Recupera propriedades de carga de corpo do elemento:
c     e(4) = rho (densidade), e(5) = gx, e(6) = gy
c
      rho = e(4)
      gx  = e(5)
      gy  = e(6)
c
c     Fator de espessura (t = 1.0 para EPD, t = e(3) para EPT)
c
      thic = 1.d0
      if (iel .eq. 2 .or. iel .eq. 4) thic = e(3)
c
c     --- ELEMENTO T3 (Integracao Analitica) ---
c     O determinante da matriz Jacobiana do T3 é igual a 2x a Área
c
      if (iel .eq. 1 .or. iel .eq. 2) then
        det = (xl(1,1)-xl(1,3))*(xl(2,2)-xl(2,3)) 
     .      - (xl(2,1)-xl(2,3))*(xl(1,2)-xl(1,3))
        area = 0.5d0 * dabs(det)
        
c       wt = Parcela de peso para CADA nó (1/3 da força total)
        wt = rho * area * thic / 3.d0
c
c       Acumula nos graus de liberdade (x, y) dos 3 nós do triângulo
        do 100 i = 1, 3
          k = (i-1)*2 + 1
          fl(k)   = fl(k)   - wt * gx
          fl(k+1) = fl(k+1) - wt * gy
...
c        
c ======================================================================
c     --- ELEMENTO Q4 (Integracao Numerica via Gauss 2x2) ---
c ======================================================================
      else if (iel .eq. 3 .or. iel .eq. 4) then
c
c       Loop nos 4 pontos de integração de Gauss (2x2)
c
        do 300 i = 1, 2
        do 300 j = 1, 2
          if (i .eq. 1 .and. j .eq. 1) then
...
c
c         1. Derivadas das funcoes de forma naturais (hr = dN/dr, hs = dN/ds)
c
          hr(1) =   (1.d0+ss) / 4.d0
          hr(2) = - (1.d0+ss) / 4.d0
          hr(3) = - (1.d0-ss) / 4.d0
          ...
          hs(4) = - (1.d0+rr) / 4.d0
c
c         2. Montagem da Matriz Jacobiana local: J = dX/d(r,s)
c
          xj(1,1) = hr(1)*xl(1,1)+hr(2)*xl(1,2)
     .            + hr(3)*xl(1,3)+hr(4)*xl(1,4)
          xj(2,1) = hs(1)*xl(1,1)+hs(2)*xl(1,2)
     .            + hs(3)*xl(1,3)+hs(4)*xl(1,4)
          xj(1,2) = hr(1)*xl(2,1)+hr(2)*xl(2,2)
     .            + hr(3)*xl(2,3)+hr(4)*xl(2,4)
          xj(2,2) = hs(1)*xl(2,1)+hs(2)*xl(2,2)
     .            + hs(3)*xl(2,3)+hs(4)*xl(2,4)
c
c         3. Determinante da Jacobiana (fator de escala de area dOmega = detJ * dr * ds)
c
          det = xj(1,1)*xj(2,2) - xj(2,1)*xj(1,2)
          
c         Peso infinitesimal do ponto atual (wi = wj = 1.0 para Gauss 2x2)
          wt  = dabs(det) * thic * rho
c
c         4. Valores das Funcoes de Forma Originais N(r,s) neste ponto de Gauss
c
          g_shape(1) = (1.d0+rr)*(1.d0+ss)/4.d0
          g_shape(2) = (1.d0-rr)*(1.d0+ss)/4.d0
          g_shape(3) = (1.d0-rr)*(1.d0-ss)/4.d0
          g_shape(4) = (1.d0+rr)*(1.d0-ss)/4.d0
c
c         5. Distribuição e acumulação da força peso nos GDL locais (x, y) dos 4 nós
c
          do 200 m = 1, 4
            k = (m-1)*2 + 1
            fl(k)   = fl(k)   - g_shape(m) * gx * wt
            fl(k+1) = fl(k+1) - g_shape(m) * gy * wt
  200     continue
  300   continue
      endif
c
