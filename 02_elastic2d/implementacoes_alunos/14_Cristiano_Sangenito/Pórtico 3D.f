...
c       ndm   = dimensao do problema (2 para 2D, 3 para 3D)
c
...
c      if (nen .ne. 3 .and. nen .ne. 4) then
c        print*, '*** ERRO: nen deve ser 3 (T3) ou 4 (Q4). Lido:', nen
c        stop
c      endif
      if (ndm .ne. 2 .and. ndm .ne. 3) then
        print*, '*** ERRO: programa valido apenas para 2D ou 3D'
        stop
      endif
c
...
c     Leitura dos dados:
c
      call rdata(a(1),m(i1),m(i2),m(i3),a(i4),a(i5),nnode,numel,numat,
     .           nen,ndm,ndf,nin)
c
...
c
      ...
      call pcg(a(i8), a(i6), m(i7), a(i14), a(i15), a(i16), a(i17),
     .         a(i18), neq)
...
c ======================================================================
  ...
  600 call elmt01_axi(e,xl,ul,fl,sl,nel,ndm,nst,isw,nin)
      return
  700 call elmt07(e,xl,ul,fl,sl,nel,ndm,nst,isw,nin)
      return
      end
...
c ======================================================================
      ...
      do 100 i = 1, nnode
        write(nout,'(i10,3e15.5)') i,(x(j,i),j=1,ndm)
  100 continue
...
c ======================================================================
      subroutine elmt07(e,x,u,p,s,nel,ndm,nst,isw,nin)
c     Elemento de Portico Espacial 3D (Beam 3D) - 2 nos, 6 GDL/no
c     nst = 12, ndm = 3
c
c     Parametros do material/secao (lidos em e):
c        e(1) = E  (Modulo de Young)
c        e(2) = G  (Modulo de Cisalhamento)
c        e(3) = A  (Area da secao transversal)
c        e(4) = J  (Inercia a torcao)
c        e(5) = Iy (Inercia em relacao ao eixo local y)
c        e(6) = Iz (Inercia em relacao ao eixo local z)
c ======================================================================
      integer nel,ndm,nst,isw,nin,i,j,k
      real*8  e(*),x(ndm,*),u(nst),p(nst),s(nst,nst)
      real*8  xl,yl,zl,length,length2,length3
      real*8  ee,gg,area,aj,aiy,aiz
      real*8  ylx,yly,ylz,zlx,zly,zlz
      real*8  nx,ny,nz,mx,my,mz,lx,ly,lz,det
      real*8  r(3,3),t(12,12),sl(12,12),aux(12,12)
      real*8  ul(12)
      
      goto (100,200,300) isw

c     ------------------------------------------------------------------
c     isw = 1: Leitura das propriedades da secao e material
c     ------------------------------------------------------------------
  100 continue
      read(nin,*) e(1),e(2),e(3),e(4),e(5),e(6)
      return

c     ------------------------------------------------------------------
c     isw = 2: Calculo da Matriz de Rigidez Rotacionada (Global)
c     ------------------------------------------------------------------
  200 continue
c     1. Geometria do elemento e comprimento
      xl = x(1,2) - x(1,1)
      yl = x(2,2) - x(2,1)
      zl = x(3,2) - x(3,1)
      length = dsqrt(xl*xl + yl*yl + zl*zl)
      
      if (length .lt. 1.d-12) then
        print*, '*** ERRO elmt07: Comprimento nulo no elemento', nel
        stop
      endif
...
c     2. Montagem da Matriz de Rigidez Local (sl) - Inicializacao com zero
      do i = 1, 12
        do j = 1, 12
          sl(i,j) = 0.d0
        enddo
...
c     Termos axiais e torcao
      sl(1,1)   =  ee*area/length
      sl(1,7)   = -ee*area/length
      sl(4,4)   =  gg*aj/length
      sl(4,10)  = -gg*aj/length
      sl(7,1)   = -ee*area/length
      sl(7,7)   =  ee*area/length
      sl(10,4)  = -gg*aj/length
      sl(10,10) =  gg*aj/length
      
c     Flexao Plano XY (Inercia Iz, deslocamento v, rotacao theta_z)
      sl(2,2)   =  12.d0*ee*aiz/length3
      sl(2,6)   =  6.d0*ee*aiz/length2
      sl(2,8)   = -12.d0*ee*aiz/length3
      sl(2,12)  =  6.d0*ee*aiz/length2
      
      sl(6,2)   =  6.d0*ee*aiz/length2
      sl(6,6)   =  4.d0*ee*aiz/length
      sl(6,8)   = -6.d0*ee*aiz/length2
      sl(6,12)  =  2.d0*ee*aiz/length
      
      sl(8,2)   = -12.d0*ee*aiz/length3
      sl(8,6)   = -6.d0*ee*aiz/length2
      sl(8,8)   =  12.d0*ee*aiz/length3
      sl(8,12)  = -6.d0*ee*aiz/length2
      
      sl(12,2)  =  6.d0*ee*aiz/length2
      sl(12,12) =  4.d0*ee*aiz/length
      sl(12,6)  =  2.d0*ee*aiz/length
      sl(12,8)  = -6.d0*ee*aiz/length2

c     Flexao Plano XZ (Inercia Iy, deslocamento w, rotacao theta_y)
      sl(3,3)   =  12.d0*ee*aiy/length3
      sl(3,5)   = -6.d0*ee*aiy/length2
      sl(3,9)   = -12.d0*ee*aiy/length3
      sl(3,11)  = -6.d0*ee*aiy/length2
      
      sl(5,3)   = -6.d0*ee*aiy/length2
      sl(5,5)   =  4.d0*ee*aiy/length
      sl(5,9)   =  6.d0*ee*aiy/length2
      sl(5,11)  =  2.d0*ee*aiy/length
      
      sl(9,3)   = -12.d0*ee*aiy/length3
      sl(9,5)   =  6.d0*ee*aiy/length2
      sl(9,9)   =  12.d0*ee*aiy/length3
      sl(9,11)  =  6.d0*ee*aiy/length2
      
      sl(11,3)  = -6.d0*ee*aiy/length2
      sl(11,11) =  4.d0*ee*aiy/length
      sl(11,5)  =  2.d0*ee*aiy/length
      sl(11,9)  =  6.d0*ee*aiy/length2

c     3. Matriz de Direcao Espacial (Cossenos Diretores)
      r(1,1) = xl / length
      r(1,2) = yl / length
      r(1,3) = zl / length

      if (dabs(r(1,1)).lt.1.d-4 .and. dabs(r(1,2)).lt.1.d-4) then
c       Elemento estritamente vertical
        r(2,1) = 0.d0
        r(2,2) = 1.d0
        r(2,3) = 0.d0
        r(3,1) = -r(1,3)
...
c       Elemento generico
        ...
        ny = 0.d0
        nz = 1.d0
        zlx = r(1,2)*nz - r(1,3)*ny
        zly = r(1,3)*nx - r(1,1)*nz
        zlz = r(1,1)*ny - r(1,2)*nx
        det = dsqrt(zlx*zlx + zly*zly + zlz*zlz)
        r(3,1) = zlx / det
        r(3,2) = zly / det
        r(3,3) = zlz / det
...
c     4. Expandir para a Matriz de Transformacao Global 12x12 (T)
      do i = 1, 12
        do j = 1, 12
          t(i,j) = 0.d0
        enddo
...
c     5. Multiplicacao de Matrizes: S = T^T * sl * T
      do i = 1, 12
        do j = 1, 12
          aux(i,j) = 0.d0
          do k = 1, 12
            aux(i,j) = aux(i,j) + sl(i,k) * t(k,j)
          enddo
        enddo
      enddo
...
c     ------------------------------------------------------------------
c     isw = 3: Pos-processamento - Calculo das forcas internas locais
c     ------------------------------------------------------------------
  300 continue
c     Recalcula comprimento e propriedades basicas para recuperar sl e T
      xl = x(1,2) - x(1,1)
      yl = x(2,2) - x(2,1)
      zl = x(3,2) - x(3,1)
      length = dsqrt(xl*xl + yl*yl + zl*zl)
...
c     Remonta sl local para a operacao p = sl * ul
      do i = 1, 12
        do j = 1, 12
          sl(i,j) = 0.d0
        enddo
...
c     Chama lku passando os deslocamentos locais para obter as forcas locais
      call lku(sl, ul, p, nst)
